...
import { getProductos, createVenta, updateVenta, getVentas, procesarCuadernoVentas } from '../../shared/services/api';
...
export const useVentas = () => {
    ...
    useEffect(() => {
        const cargarOrdenesPersistidas = async () => {
            ...
            try {
                const storedValue = await AsyncStorage.getItem(`vestaMarket_ordenes_${negocioId}`);
                const storedActiveId = await AsyncStorage.getItem(`vestaMarket_active_order_id_${negocioId}`);
    ...
    useEffect(() => {
        const guardarOrdenes = async () => {
            if (!negocioId) return;
            try {
                await AsyncStorage.setItem(`vestaMarket_ordenes_${negocioId}`, JSON.stringify(ordenes));
                await AsyncStorage.setItem(`vestaMarket_active_order_id_${negocioId}`, activeOrderId);
            } catch (err) {
