...
export const useUsuarios = () => {
    ...
    const handleCreateNegocio = async (data: { nombre: string, categoria: 'COMIDA' | 'FRUTERIA', telefono?: string }) => {
    ...
    const handleDeleteNegocio = (id: string) => {
        if (negocios.length <= 1) {
            setError('No puedes eliminar tu único local. Vesta Market requiere al menos uno activo.');
            return;
        }
